...
<div class="header">
<p>Ol\xE1 Deva! Seja bem vinda, esta comunidade foi criada com o objetivo de conectar voc\xEA com as vagas de emprego disponiveis na \xE1rea da tecnologia.</p>
</div>
`;
...
function $a(t2, e, n = {}) {
  ...
  return t2.asyncQueue.enqueueAndForget(async () => function(t3, e2, n2, s2, i) {
    const r = new wa({
      next: (n3) => {
        e2.enqueueAndForget(() => gc(t3, o)), n3.fromCache && "server" === s2.source ? i.reject(new L(B.UNAVAILABLE, 'Failed to get documents from server. (However, these documents may exist in the local cache. Run again without setting source to "server" to retrieve the cached documents.)')) : i.resolve(n3);
      },
      error: (t4) => i.reject(t4)
    }), o = new Tc(n2, r, {
      includeMetadataChanges: true,
...
function Jl(t2) {
  t2 = Qa(t2, Ya);
  const e = Qa(t2.firestore, ch), n = lh(e), s = new zl(e);
  return ol(t2._query), $a(n, t2._query).then((n2) => new Kl(e, s, t2, n2));
}
...
const createPostData = (postContent) => {
  const date = new Date();
  return {
    message: postContent,
    userId: getUser().uid,
...
const newPost = async (postContent) => {
  ...
  const docRef = nf(Xa(db, "posts"), dataPost);
  return docRef;
};
async function readAllPosts() {
  const queryOrder = al(Xa(db, "posts"), ml("publishDate"));
  const querySnapshot = await Jl(queryOrder);
  const posts = [];
...
const readOnePost = async (idPost) => {
  const post = await jl(eh(db, "posts", idPost));
  return {
    id: post.id,
    ...post.data()
  };
...
const deletePost = (idPost) => {
  ef(eh(db, "posts", idPost));
};
function likePost(postId, userUID) {
  const docRef = eh(db, "posts", postId);
  tf(docRef, {
    likes: gf(userUID)
  });
}
function deslikePost(postId, userUID) {
  const docRef = eh(db, "posts", postId);
  tf(docRef, {
    likes: yf(userUID)
  });
}
async function signIn(name2, email, password) {
  return createUserWithEmailAndPassword(auth, email, password).then((userCredential) => {
    const user = userCredential.user;
    return updateProfile(user, { displayName: name2 });
  });
}
...
function getErrorMessage(error) {
  switch (error.code) {
    ...
    case "auth/invalid-email":
      return "O endere\xE7o de e-mail n\xE3o \xE9 v\xE1lido.";
    case "auth/email-already-in-use":
      return "O e-mail inserido j\xE1 possui cadastro.";
    case "auth/wrong-password":
      return "Senha incorreta.";
    case "auth/invalid-display-name":
      return "O nome do usu\xE1rio \xE9 inv\xE1lido.";
    case "auth/weak-password":
      return "A senha deve ter 6 ou mais caracteres.";
    default:
      return `Aconteceu um erro n\xE3o identificado, por favor entre em contato com as desenvolvedoras e indique o c\xF3digo que aparecer\xE1 a seguir: ${error.code}`;
  }
}
const login = () => {
  ...
  <div class="container-login">
        ...
        <div class="content-box-login">        
            <div class="form-box-login">
                    <p>Para fazer parte dessa comunidade voc\xEA precisa estar logado</p>              
                <form class="form-login">
                    ...
                    <div class="input-box">
                       <p>N\xE3o Tem Uma Conta?<a href="" id="cadastrar">  Cadastrar</a></p>                       
                    </div>
                </form>
                <div class="box-google-title">
                  <h4>Ou se preferir, voc\xEA pode logar com</h4>
                <div>
...
`;
  ...
  btnLogin.addEventListener("click", (e) => {
    e.preventDefault();
    login$1(email.value, password.value).then(() => {
      redirect("#feed");
    }).catch((error) => {
...
function validationRegister(name2, email, password, passwordTwo) {
  const regex = /\S+@\S+\.\S+/;
  const validateEmail = regex.test(email);
  if (name2 === "") {
    return {
      message: "Voc\xEA precisa preencher o campo de nome.",
      source: "name"
    };
  }
  if (name2.length < 8) {
  ...
  if (email === "") {
    return {
      message: "Voc\xEA precisa preencher o campo de email.",
      source: "email"
    };
  }
  if (password === "") {
    return {
      message: "Voc\xEA precisa preencher o campo de senha.",
      source: "password"
    };
  }
  ...
  if (passwordTwo === "") {
    return {
      message: "Voc\xEA precisa preencher o campo de confirme sua senha.",
      source: "passwordTwo"
    };
  }
  ...
  if (validateEmail === false) {
    return {
      message: "Insira um e-mail v\xE1lido (ex: seunome@email.com)",
      source: "email"
    };
  }
  return null;
...
`;
  ...
  const name2 = containerRegister.querySelector(".input-register-name");
  ...
  function errorValidation() {
    containerRegister.querySelectorAll(".error-name, .error-email, .error-password, .error-passwordTwo").forEach((i) => {
      i.innerHTML = "";
    });
    containerRegister.querySelectorAll(".input-register-name, .input-register-email, .input-register-password, .input-register-passwordTwo").forEach((i) => {
...
<nav id="footer">
  ...
  const sobre2 = containerFooter.querySelector("#sobre");
  sobre2.addEventListener("click", () => {
    redirect("#sobre");
  });
  return containerFooter;
}
const coracaoDeslike = "/assets/heart.13a0064f.svg";
const coracaoLike = "/assets/heart-fill.565c1330.svg";
...
function publish(timelinePost) {
  ...
  <div class='publish'>
    ...
    <textarea class='post-publish' id='post-publish' cols='60' rows='10' placeholder="Digite aqui sua Publica\xE7\xE3o" style='resize:none'></textarea>
  ...
  btnPublish.addEventListener("click", async () => {
    ...
    if (postContent.length > 0) {
      ...
      messageTextarea.innerHTML = "Publica\xE7\xE3o realizada com sucesso!";
      postPublish.innerHTML = "";
    } else {
      messageTextarea.innerHTML = "Campo vazio, por favor digite sua publica\xE7\xE3o";
    }
    postPublish.value = "";
  });
  showPosts.then((post) => {
    post.forEach((post2) => {
      allPosts.appendChild(timelinePosts(post2));
    });
  });
...
function headerSobre() {
  ...
  <h1>
  Saiba mais sobre o Ol\xE1 Devas!
 </h1>
  </div>
  `;
  containerHeader.innerHTML = templateHeader;
...
const sobre = () => {
  ...
  <div class="container-sobre">
   <p>Desenvolvido por <a class='link' href="https://github.com/daianedosanjos?tab=repositories" target="_blank"> Daiane dos Anjos</a> e  <a href="https://github.com/s2arahrv?tab=repositories" target="_blank"> Sarah Rodrigues</a>, o Ol\xE1 devas \xE9 uma rede social para uso exclusivo de mulheres do setor tecnol\xF3gico.
  Com o intu\xEDto de diminuir o gap entre homens x mulheres no setor, o Ol\xE1 Devas foi criado para facilitar a procura de vagas.
  Nela a usu\xE1ria poder\xE1 criar uma conta de acesso, logar-se com ela, criar, editar, deletar e dar likes em publica\xE7\xF5es.</p> 
...
const main$1 = document.querySelector("#root");
const init = () => {
